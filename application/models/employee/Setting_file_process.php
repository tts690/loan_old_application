...
class Setting_file_process extends MY_Model {
    ...
    public function update_generate_details($data) {
     ...
     public function get_generate_details($last_inserted_id){
        ...
        $this->db->where('file_generated_details.role','emp');
...
//        $this->db->distinct();
//        $this->db->group_by('generate_file');
        $this->db->where('role',"emp");
        $this->db->where('income_source_id',$income_source);
        //$this->db->join('disbursment_document', 'disbursment_document.generate_file_id = generate_file.generate_file_id');
        $query = $this->db->get();
//        echo $this->db->last_query();
//        die();
        $data = $query->result();
