...
class Setting_disbursement_process extends MY_Model {
    ...
    public function update_disbursement_document($data) {
        ...
        //die();
        return TRUE;
        //$this->db->insert('disbursment_document', $data);
        //return TRUE;
    }
...
//    }
    ...
    public function select_disbursement_doc_id($generated_id){
        $this->db->select('*');
        $this->db->from('disbursment_document');
        $this->db->where('role',"DSA");
        $this->db->where('generate_file_id',$generated_id);
        $this->db->join('file_disbursment_details', 'file_disbursment_details.generate_file_id = disbursment_document.generate_file_id');
        echo $this->db->last_query();
        die();
        $query1 = $this->db->get();
        $data1 = $query1->result();
